...
class Exp_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            # self.model.load_state_dict(torch.load("checkpoints/timer_Era5_Pretrain/checkpoint.pth"))
            
            
        # watcher = ww.WeightWatcher(model=self.model,framework="pytorch") 
        # details = watcher.analyze() 
        # summary = watcher.get_summary(details) 
        # print(summary) 
        # details.to_excel("ww-output.xlsx", sheet_name='Sheet1') 
        # exit(0)
        preds = []
        trues = []
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    gt = np.array(true[0, :, -1])[:200]
                    pd = np.array(pred[0, :, -1])[:200]
                    #visual(gt, pd, os.path.join(dir_path, f'{i}.pdf'))
                    #print(f"Saving plot to: {os.path.join(folder_path, f'{i}.pdf')}")
                    visual(gt, pd, os.path.join(dir_path, f'{i}.pdf'))
